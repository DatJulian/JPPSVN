			...
			if(string.IsNullOrWhiteSpace(path)) {
				...
				worker.DoWork += (object s, DoWorkEventArgs args) => {
					using(SvnClient client = new SvnClient()) {
						client.Progress += (object sender, SvnProgressEventArgs e) => { worker.ReportProgress((int) e.Progress, e); };

						try {
							client.CheckOut(SvnUriTarget.FromString(url), path, out SvnUpdateResult res);
